...
import { GATEWAY_ADDRESS, POLYMARKET_ADDRESS } from "./const";
...
export async function run(provider: NetworkProvider) {
  ...
  const destination = beginCell().store(storeDestination(polyMarketCTF_ID)).endCell();

  const settle_query_id = 1n;
  const usd_amount = toNano("0.0001");
  ...
  const expiration = BigInt(Math.round(new Date().getTime() / 1000 + 60) + 120);
  ...
  const msg_hash = await skateGateway.getPayloadHash(settle_query_id, POLYMARKET_ADDRESS, payload, expiration);

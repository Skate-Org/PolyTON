...
import { TESTNET_GATEWAY_ADDRESS, TESTNET_USDT_ADDRESS } from "../const";
...
export async function run(provider: NetworkProvider) {
  ...
  const polyMarket = provider.open(await PolyMarket.fromInit(owner, TESTNET_GATEWAY_ADDRESS));

  await polyMarket.send(provider.sender(), { value: toNano("0.01") }, { $$type: "Deploy", queryId: 0n });
